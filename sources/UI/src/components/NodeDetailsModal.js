          ...
          <div className="node-info-section">
            ...
            <div className="info-grid">
              ...
              <div className="info-item">
                ...
                <span className="value">{safeString(node.metadata?.columns || 0)}</span>
              </div>
              <div className="info-item">
          ...
          {node.columns && Object.keys(node.columns).length > 0 && (
            <div className="columns-section">
              ...
              <div className="columns-grid">
                {Object.entries(node.columns).map(([columnName, columnData]) => (
                  <div key={columnName} className="column-card">
                    ...
                    <div className="column-stats">
                      <div className="stat-item">
                        ...
                        <span className="stat-value">{safeString(columnData.position)}</span>
                      </div>
                      <div className="stat-item">
                      ...
                      <div className="stat-item">
                        ...
                        <span className="stat-value">{safeString(columnData.unique_count)}</span>
                      </div>
                      <div className="stat-item">
                        <span className="stat-label">Null Count:</span>
                        <span className="stat-value">{safeString(columnData.null_count)}</span>
                      </div>
                      {columnData.min_value !== null && (
                        <div className="stat-item">
                          <span className="stat-label">Min Value:</span>
                          <span className="stat-value">{safeString(columnData.min_value)}</span>
                        </div>
                      )}
                      {columnData.max_value !== null && (
                        <div className="stat-item">
                          <span className="stat-label">Max Value:</span>
                          <span className="stat-value">{safeString(columnData.max_value)}</span>
                        </div>
                      )}
                      {columnData.max_length && (
                        <div className="stat-item">
                          <span className="stat-label">Max Length:</span>
                          <span className="stat-value">{safeString(columnData.max_length)}</span>
                        </div>
                      )}
                    </div>

                    {columnData.sample_values && columnData.sample_values.length > 0 && (
                      <div className="sample-values">
                        <span className="sample-label">Sample Values:</span>
                        <div className="sample-list">
                          {columnData.sample_values.slice(0, 5).map((value, index) => (
