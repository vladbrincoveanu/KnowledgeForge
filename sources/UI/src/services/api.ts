...
export const businessContextAPI = {
  ...
  getSnapshotDiff: async (
    ...
    try {
      const response: AxiosResponse<BusinessSnapshotDiffPayload> =
        await api.get(`/api/v1/context/${encodeURIComponent(systemId)}/diff`);
      return response.data;
    } catch {
      const fallback =
  ...
  updateOverride: async (
    ...
    try {
      ...
      const store =
        fallbackBusinessContextStore[systemId] ||
        fallbackBusinessContextStore.wps;
      const idx = store.fields.findIndex(
        field => field.field_path === fieldPath
      );
      if (idx < 0) {
