...
vi.mock('reactflow', async () => {
  ...
  return {
    ...
    useReactFlow: () => ({ fitView: vi.fn(), setNodes: vi.fn(), setEdges: vi.fn() }),
    Background: () => null,
    Controls: () => null,
    MiniMap: () => null,
    Handle: () => null,
...
vi.mock('./CustomNode', () => ({ default: () => <div data-testid="custom-node" /> }));
vi.mock('./ContainerNode', () => ({ default: () => <div data-testid="container-node" /> }));
vi.mock('./C4Edge', () => ({ default: () => null }));
vi.mock('./components/ArchitectureHeader', () => ({
  default: () => <div data-testid="architecture-header">ArchitectureHeader</div>,
}));
vi.mock('./components/FiltersSidebar', () => ({
  default: () => <div data-testid="filters-sidebar">FiltersSidebar</div>,
}));
...
vi.mock('dagre', () => ({ default: { graphlib: { Graph: vi.fn() }, layout: vi.fn() } }));
...
describe('CodeArchitectureViewer', () => {
  ...
  test('renders FiltersSidebar sub-component', async () => {
    mockGetArchitecture.mockResolvedValueOnce(emptyArchitecture);
    render(<CodeArchitectureViewer />);
    await waitFor(() => expect(mockGetArchitecture).toHaveBeenCalled());
