...
const CATEGORY_CONFIG: Record<ContainerCategory, { badge: string; color: string; border: string }> = {
...
const ContainerNode: React.FC<ContainerNodeProps> = ({ data }) => {
  ...
  return (
    <div className={`container-node container-node--${category}`} style={{ borderColor: config.border }}>
      <Handle type="target" position={Position.Left} />
      <div className="container-badge" style={{ backgroundColor: config.color }}>
        {config.badge}
      </div>
      <div className="container-header">
        <div className="container-label">{data.label}</div>
