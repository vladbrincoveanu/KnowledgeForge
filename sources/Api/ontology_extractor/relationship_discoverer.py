...
class RelationshipDiscoverer:
    ...
    def __init__(self, llm_manager: Optional[LLMManager] = None, 
                 use_sbert: bool = True, cache_dir: Optional[str] = None):
    ...
    def discover_relationships(self, file_path: str, entities: List[Entity], 
                             columns: List[ColumnProfile], config: Dict[str, Any]) -> List[Relationship]:
    ...
    def _discover_foreign_key_relationships(self, file_path: str, entities: List[Entity], 
                                          columns: List[ColumnProfile], 
                                          config: Dict[str, Any]) -> List[Relationship]:
        ...
        for col in columns:
            ...
            if any(pattern in col_lower for pattern in self.relationship_patterns['foreign_key']):
    ...
    def _analyze_foreign_key_candidate(self, file_path: str, col1: ColumnProfile, 
                                      col2: ColumnProfile, entity_columns: Dict[str, List[Entity]], 
                                      config: Dict[str, Any]) -> List[Relationship]:
        """Analyze if two columns have a foreign key relationship."""
        relationships = []
        
        # Check if column names look like auto-generated names that might cause issues
        if (col1.name.startswith('column') and '+' in col1.name) or (col2.name.startswith('column') and '+' in col2.name):
            logger.warning(f"Auto-generated column names detected in FK analysis: '{col1.name}', '{col2.name}' - skipping")
            return []
    ...
    def _calculate_fk_confidence(self, overlap_percentage: float, unique_col1: int, 
                                unique_col2: int, total_rows: int) -> float:
    ...
    def _find_representative_entity(self, column: ColumnProfile, 
                                   entity_columns: Dict[str, List[Entity]]) -> Optional[Entity]:
    ...
    def _get_fk_evidence(self, file_path: str, source_col: ColumnProfile, 
                         target_col: ColumnProfile) -> List[Dict[str, Any]]:
        """Get sample evidence for foreign key relationship."""
        try:
            # Debug: log the column names being used
            logger.debug(f"Getting FK evidence for columns: '{source_col.name}' and '{target_col.name}'")
            
            # Validate column names to prevent SQL injection and errors
            if not source_col.name or not target_col.name:
                logger.warning("Invalid column names for FK evidence")
                return []
            
            # Check if column names contain invalid characters
            if any(char in source_col.name + target_col.name for char in ['+', '-', '*', '/', '(', ')', '"', "'"]):
                logger.warning(f"Column names contain invalid characters: '{source_col.name}', '{target_col.name}'")
                return []
            
            # Check if column names look like auto-generated names that might cause issues
            if (source_col.name.startswith('column') and '+' in source_col.name) or (target_col.name.startswith('column') and '+' in target_col.name):
                logger.warning(f"Auto-generated column names detected: '{source_col.name}', '{target_col.name}' - skipping")
                return []
            ...
            logger.debug(f"Executing FK evidence query: {query}")
            ...
            logger.warning(f"Error getting FK evidence for columns '{source_col.name}' and '{target_col.name}': {e}")
            return []
    
    def _discover_semantic_relationships_sbert(self, entities: List[Entity], 
                                             columns: List[ColumnProfile], 
                                             config: Dict[str, Any]) -> List[Relationship]:
                ...
                for j in range(i + 1, len(embeddings)):
                    ...
                    if similarity >= config.get('semantic_similarity_threshold', 0.7):
                        ...
                        relationship = Relationship(
                            ...
                            source_columns=[entity1.source_column, entity2.source_column]
                            if entity1.source_column and entity2.source_column else []
                        )
                        relationships.append(relationship)
    ...
    def _infer_relationship_type_from_similarity(self, entity1: Entity, 
                                               entity2: Entity, similarity: float) -> str:
    ...
    def _discover_llm_relationships(self, file_path: str, entities: List[Entity], 
                                   columns: List[ColumnProfile], 
                                   config: Dict[str, Any]) -> List[Relationship]:
        ...
        try:
            ...
            for entity in entities:
                if entity.source_column:
                    entities_by_column[entity.source_column].append(entity)
                ...
                for col2_name, col2_entities in entities_by_column.items():
                    if col1_name != col2_name:
                        llm_rels = self._analyze_column_relationship_with_llm(
                            file_path, col1_name, col2_name, col1_entities, col2_entities, config
                        )
                        relationships.extend(llm_rels)
    ...
    def _analyze_column_relationship_with_llm(self, file_path: str, col1_name: str, 
                                            col2_name: str, col1_entities: List[Entity], 
        ...
        try:
            ...
            if relationship_info and relationship_info.get('confidence', 0) >= config.get('relationship_threshold', 0.6):
                ...
                for entity1 in col1_entities[:5]:  # Limit to avoid too many relationships
                    for entity2 in col2_entities[:5]:
                        # Create relationship from LLM analysis
                        relationship = Relationship(
                            id=f"llm_{col1_name}_{col2_name}_{hash(f'{col1_name}_{col2_name}')}",
                            source_entity_id=entity1.id,
                            target_entity_id=entity2.id,
                            relationship_type=relationship_info.get('type', 'unknown'),
                            attributes={
                                "llm_analysis": relationship_info,
                                "evidence": sample_data,
                                "extraction_method": "llm_inference"
                            },
                            confidence=relationship_info.get('confidence', 0.7),  # Use .get() for dictionary access
    ...
    def _get_sample_data_for_llm(self, file_path: str, col1_name: str, 
                                col2_name: str) -> List[Dict[str, Any]]:
        """Get sample data for LLM relationship analysis."""
        try:
            # Debug: log the column names being used
            logger.debug(f"Getting sample data for columns: '{col1_name}' and '{col2_name}'")
            
            # Validate column names to prevent SQL errors
            if not col1_name or not col2_name:
                logger.warning("Invalid column names for sample data")
                return []
            
            # Check if column names contain invalid characters
            if any(char in col1_name + col2_name for char in ['+', '-', '*', '/', '(', ')', '"', "'"]):
                logger.warning(f"Column names contain invalid characters: '{col1_name}', '{col2_name}'")
                return []
            
            # Check if column names look like auto-generated names that might cause issues
            if (col1_name.startswith('column') and '+' in col1_name) or (col2_name.startswith('column') and '+' in col2_name):
                logger.warning(f"Auto-generated column names detected: '{col1_name}', '{col2_name}' - skipping")
                return []
            ...
            logger.debug(f"Executing sample data query: {query}")
            ...
            logger.debug(f"Found {len(sample_data)} sample data rows")
            return sample_data
            
        except Exception as e:
            logger.warning(f"Error getting sample data for columns '{col1_name}' and '{col2_name}': {e}")
            return []
    
    def _discover_hierarchical_relationships(self, entities: List[Entity], 
                                           columns: List[ColumnProfile], 
                                           config: Dict[str, Any]) -> List[Relationship]:
        ...
        for column in columns:
            if column.data_type == DataType.CATEGORICAL:
                col_entities = [e for e in entities if e.source_column == column.name]
                
                if len(col_entities) > 1:
                    # Try to find hierarchical patterns
                    hierarchical_rels = self._find_hierarchical_patterns(col_entities, column)
                    relationships.extend(hierarchical_rels)
        
        return relationships
    
    def _find_hierarchical_patterns(self, entities: List[Entity], 
                                  column: ColumnProfile) -> List[Relationship]:
    ...
    def _discover_temporal_relationships(self, file_path: str, entities: List[Entity], 
                                       columns: List[ColumnProfile], 
                                       config: Dict[str, Any]) -> List[Relationship]:
        ...
        temporal_entities = [
            e for e in entities 
            if e.entity_type == "temporal" or any(
    ...
    def _detect_relationship_cardinality(self, file_path: str, relationships: List[Relationship], 
                                       columns: List[ColumnProfile], 
                                       config: Dict[str, Any]) -> List[Relationship]:
        """Detect relationship cardinality (1:1, 1:n, n:m)."""
        for relationship in relationships:
            if "source_column" in relationship.attributes and "target_column" in relationship.attributes:
                source_col = relationship.attributes["source_column"]
                target_col = relationship.attributes["target_column"]
                
                cardinality = self._calculate_cardinality(file_path, source_col, target_col)
                relationship.attributes["cardinality"] = cardinality
                
                # Adjust confidence based on cardinality consistency
                if cardinality == "1:1" and relationship.attributes.get("overlap_percentage", 0) >= 0.95:
                    relationship.confidence = min(relationship.confidence + 0.1, 1.0)
                elif cardinality == "n:m" and relationship.attributes.get("overlap_percentage", 0) <= 0.3:
                    relationship.confidence = min(relationship.confidence + 0.05, 1.0)
        
        return relationships
    
    def _calculate_cardinality(self, file_path: str, source_col: str, target_col: str) -> str:
        """Calculate cardinality between two columns."""
        try:
            # Check if column names look like auto-generated names that might cause issues
            if (source_col.startswith('column') and '+' in source_col) or (target_col.startswith('column') and '+' in target_col):
                logger.warning(f"Auto-generated column names detected in cardinality calculation: '{source_col}', '{target_col}' - skipping")
                return "unknown"
            query = f"""
            SELECT 
    ...
    def _detect_graph_cycles(self, relationships: List[Relationship]) -> List[Relationship]:
        ...
        try:
            ...
            if cycles:
                ...
                for cycle in cycles:
                    if len(cycle) > 1:
                        ...
                        for i in range(len(cycle)):
                            ...
                            for rel in relationships:
                                if (rel.source_entity_id == source and 
                ...
                logger.info(f"Removed {len(relationships) - len(filtered_relationships)} cycle-creating relationships")
    ...
    def _discover_co_occurrence_relationships(self, file_path: str, entities: List[Entity], 
                                           columns: List[ColumnProfile], 
                                           config: Dict[str, Any]) -> List[Relationship]:
    ...
    def _get_entity_column_mapping(self, entities: List[Entity]) -> Dict[str, List[Entity]]:
        """Create mapping from column names to entities."""
        mapping = defaultdict(list)
        for entity in entities:
            if entity.source_column:
                mapping[entity.source_column].append(entity)
        return dict(mapping)
    
    def _analyze_column_relationship(self, file_path: str, col1: ColumnProfile, 
                                   col2: ColumnProfile, entity_columns: Dict[str, List[Entity]], 
                                   config: Dict[str, Any]) -> List[Relationship]:
        ...
        # Check if column names look like auto-generated names that might cause issues
        if (col1.name.startswith('column') and '+' in col1.name) or (col2.name.startswith('column') and '+' in col2.name):
            logger.warning(f"Auto-generated column names detected in column relationship analysis: '{col1.name}', '{col2.name}' - skipping")
            return []
    ...
    def _find_entity_by_value(self, value: Any, entities: List[Entity]) -> Optional[Entity]:
    ...
    def _discover_semantic_relationships(self, entities: List[Entity], 
                                       columns: List[ColumnProfile], 
                                       config: Dict[str, Any]) -> List[Relationship]:
    ...
    def _analyze_semantic_relationship(self, entities1: List[Entity], entities2: List[Entity], 
                                     type1: str, type2: str, 
                                     config: Dict[str, Any]) -> List[Relationship]:
        ...
        try:
            relationship_type = self.llm_manager.suggest_relationship_type(type1, type2)
        except Exception as e:
    ...
    def get_relationship_statistics(self, relationships: List[Relationship]) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_common_relationship_type": max(type_counts.items(), key=lambda x: x[1])[0] if type_counts else None
        }
    
    def get_relationship_evidence(self, relationship: Relationship, file_path: str) -> Dict[str, Any]:
        ...
        if extraction_method == "foreign_key_analysis":
            evidence["fk_evidence"] = {
                ...
                "unique_source_values": relationship.attributes.get("unique_source_values"),
                "unique_target_values": relationship.attributes.get("unique_target_values")
            }
        
        elif extraction_method == "sbert_similarity":
            evidence["semantic_evidence"] = {
                "semantic_similarity": relationship.attributes.get("semantic_similarity"),
            ...
            evidence["co_occurrence_evidence"] = {
                "co_occurrence_count": relationship.attributes.get("co_occurrence_count"),
    ...
    def export_relationships_to_graph(self, relationships: List[Relationship], 
        ...
        for entity in entities:
            ...
            graph_data["metadata"]["node_types"][entity_type] = graph_data["metadata"]["node_types"].get(entity_type, 0) + 1
        ...
        for relationship in relationships:
            edge = {
                ...
                "extraction_method": relationship.attributes.get("extraction_method", "unknown"),
    ...
    def validate_relationships(self, relationships: List[Relationship], 
        ...
        for relationship in relationships:
            ...
            if relationship.source_entity_id not in entity_ids:
                is_valid = False
                errors.append(f"Source entity {relationship.source_entity_id} not found")
            
            if relationship.target_entity_id not in entity_ids:
                is_valid = False
                errors.append(f"Target entity {relationship.target_entity_id} not found")
        ...
        if validation_results["total_relationships"] > 0:
            validation_results["quality_metrics"] = {
                "validity_rate": validation_results["valid_relationships"] / validation_results["total_relationships"],
                ...
                "confidence_distribution": {
                    ...
                    "medium": len([r for r in relationships if 0.6 <= r.confidence < 0.8]),
