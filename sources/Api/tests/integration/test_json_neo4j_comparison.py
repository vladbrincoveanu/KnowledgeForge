...
def load_extraction_result():
    result_path = (
        Path(__file__).parent.parent.parent / "UI" / "e2e" / ".extraction-result.json"
    )
...
class TestJsonNeo4jComparison:
    ...
    def test_container_count_matches(self, setup_neo4j):
        ...
        containers = [
            c for c in data.get("containers", []) if not c.get("is_infrastructure_only")
        ]
        ...
        ), f"Neo4j {actual_count} vs JSON {expected_count}"
    ...
    def test_external_system_count_matches(self, setup_neo4j):
        ...
        result = setup_neo4j.query(
            "MATCH (n:ExternalSystem) RETURN count(n) as count", {}
        )
        actual_count = result[0]["count"]
        assert (
            actual_count == expected_count
    ...
    def test_system_count_is_one(self, setup_neo4j):
        ...
        result = setup_neo4j.query(
            "MATCH (n:System {id: $id}) RETURN count(n) as count",
            {"id": f"context:{sys_name}"},
        )
        assert (
            result[0]["count"] == 1
        ), f"Expected 1 System node, got {result[0]['count']}"
        all_systems = setup_neo4j.query("MATCH (n:System) RETURN count(n) as count", {})
        assert (
            all_systems[0]["count"] == 1
        ), f"Expected exactly 1 System node in total, got {all_systems[0]['count']}"
    ...
    def test_provided_relationships_match_evidence(self, setup_neo4j):
        ...
        expected_evidence = sum(len(dep.get("evidence") or []) for dep in ext_deps)
        result = setup_neo4j.query(
            "MATCH ()-[r:PROVIDED]->() RETURN count(r) as count", {}
        )
        actual = result[0]["count"]
        assert actual == expected_evidence
    ...
    def test_container_relationships_match_json(self, setup_neo4j):
        ...
        neo4j_ids = {
            r["rel_id"] for r in result if r["rel_id"].startswith("container:")
        }
        expected_ids = {
