...
class ColumnProfile:
    ...
    def __post_init__(self):
        ...
        if self.unique_count > 0 and hasattr(self, "total_count"):
            self.uniqueness = (
                self.unique_count / self.total_count
    ...
    def __post_init__(self):
        ...
        if self.total_cells > 0:
            self.data_completeness = (
                self.total_cells - self.missing_cells
            ) / self.total_cells
...
def calculate_column_statistics(
    series: pd.Series, data_type: DataType
) -> dict[str, Any]:
    ...
    if data_type in [DataType.INTEGER, DataType.FLOAT]:
        ...
        stats["least_common"] = (
            series.value_counts().index[-1] if len(series.value_counts()) > 0 else None
        )
