...
from app.domain.models.entities import ColumnProfile, DatasetProfile, DataType
...
class DataProfiler:
    ...
    def profile_dataset(
        ...
        try:
            ...
            for col_name in df.columns:
                col_profile = self._profile_column_comprehensive(
                    df, col_name, file_path
                )
            ...
            logger.info(
                f"Successfully profiled dataset with {len(df)} rows and {len(df.columns)} columns"
            )
            return profile
    ...
    def _infer_semantic_type(
        self, column_name: str, col_data: pd.Series
    ) -> dict[str, Any]:
    ...
    def _analyze_cardinality(
        self, unique_count: int, total_count: int
    ) -> dict[str, Any]:
    ...
    def _analyze_value_distribution(
        self, col_data: pd.Series, data_type: DataType
    ) -> dict[str, Any]:
    ...
    def _analyze_potential_keys(
        self, col_data: pd.Series, unique_count: int, total_count: int
    ) -> dict[str, Any]:
        ...
        if not key_analysis["is_potential_key"] and unique_ratio > 0.5:
            key_analysis["reasons"].append(
                "Moderate uniqueness - potential composite key component"
            )
            ...
            stats.update(
                {
                    ...
                    "skewness": (
                        float(clean_data.skew()) if len(clean_data) > 2 else None
                    ),
                    "kurtosis": (
                        float(clean_data.kurtosis()) if len(clean_data) > 2 else None
                    ),
                ...
                stats.update(
                    {
                        ...
                        "date_range_years": (clean_data.max() - clean_data.min()).days
                        / 365.25,
                    }
                )
            ...
            stats.update(
                {
                    ...
                    "simpson_diversity": self._calculate_simpson_diversity(
                        value_counts
                    ),
                }
            )
    ...
    def _get_smart_sample_values(
        ...
        if data_type in [DataType.INTEGER, DataType.FLOAT]:
            ...
            random_samples = clean_data.sample(
                n=min(max_samples - len(samples), 10)
            ).tolist()
            ...
            random_values = clean_data.sample(
                n=min(max_samples - len(top_values), max_samples // 2)
            ).tolist()
    ...
    def _detect_encoding(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        encoding_info = {"potential_issues": [], "recommendations": []}
        ...
        if mixed_types:
            encoding_info["potential_issues"].append(
                f"Mixed data types in columns: {mixed_types}"
            )
            encoding_info["recommendations"].append(
                "Consider data type standardization"
            )
            ...
            for value in sample_values:
                if isinstance(value, str):
                    try:
                        ...
                        encoding_info["potential_issues"].append(
                            f"Encoding issues in column: {col}"
                        )
                        encoding_info["recommendations"].append(
                            "Check file encoding and consider UTF-8"
                        )
                        break
    ...
    def _estimate_memory_optimization(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        for col in df.columns:
            ...
            if col_data.dtype == "object":
                ...
                if col_data.dtype == "object":
                    ...
                    unique_ratio = col_data.nunique() / len(col_data)
                    if unique_ratio < 0.5:
                        optimized_memory += col_data.astype("category").memory_usage(
                            deep=True
                        )
                        optimization["recommendations"].append(
                            f"Convert {col} to category (unique ratio: {unique_ratio:.2f})"
                        )
                    else:
                ...
                if col_data.dtype == "int64":
                    if col_data.min() >= 0 and col_data.max() < 255:
                        optimized_memory += col_data.astype("uint8").memory_usage(
                            deep=True
                        )
                        optimization["recommendations"].append(
                            f"Convert {col} to uint8"
                        )
                    elif col_data.min() >= -32768 and col_data.max() < 32767:
                        optimized_memory += col_data.astype("int16").memory_usage(
                            deep=True
                        )
                        optimization["recommendations"].append(
                            f"Convert {col} to int16"
                        )
                    else:
                        optimized_memory += col_data.memory_usage(deep=True)
                else:
        ...
        optimization["savings_mb"] = (
            optimization["current_memory_mb"] - optimization["optimized_memory_mb"]
        )
    ...
    def _get_cached_profile(self, file_path: str) -> Optional[DatasetProfile]:
        ...
        try:
            ...
            if (
                cache_entry.file_hash == current_file_info["file_hash"]
                and cache_entry.file_size == current_file_info["file_size"]
                and cache_entry.file_mtime == current_file_info["file_mtime"]
            ):
    ...
    def _cache_profile(
        self, file_path: str, profile: DatasetProfile, file_info: dict[str, Any]
    ):
    ...
    def profile_incremental(
        self, file_path: str, last_profile: DatasetProfile, sample_size: int = 1000
    ) -> dict[str, Any]:
        ...
        if current_profile.row_count != last_profile.row_count:
            ...
            changes["row_count_change"] = (
                current_profile.row_count - last_profile.row_count
            )
    ...
    def get_profile_summary(self, profile: DatasetProfile) -> dict[str, Any]:
        ...
        summary = {
            ...
            "null_analysis": {
                "columns_with_nulls": len(
                    profile.metadata.get("columns_with_nulls", [])
                ),
        ...
        for col in profile.columns:
            ...
            for pattern_name, _pattern_info in patterns.items():
