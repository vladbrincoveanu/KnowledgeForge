...
class EmbeddingManager:
    ...
    def __init__(self, config: Optional[EmbeddingConfig] = None):
        ...
        logger.info(
            f"EmbeddingManager initialized with model: {self.config.model_name}"
        )
        ...
        try:
            if model_name not in self.models:
                ...
                model = SentenceTransformer(
                    model_name, cache_folder=self.config.cache_dir
                )
    ...
    def generate_embeddings(
        ...
        try:
            ...
            logger.debug(
                f"Generated embeddings for {len(texts)} texts in {processing_time:.3f}s"
            )
            return embeddings
    ...
    def _save_cache_metadata(self, cache_key: str, metadata: dict[str, Any]):
        ...
        try:
            metadata_path = os.path.join(
                self.config.cache_dir, f"{cache_key}_metadata.json"
            )
    ...
    def similarity_search(
        ...
        try:
            ...
            similarities = (
                util.pytorch_cos_sim(query_embedding, candidate_embeddings)[0]
                .cpu()
                .numpy()
            )
            ...
            logger.info(
                f"Similarity search completed: {len(matches)} matches above threshold {threshold}"
            )
            return result
    ...
    def find_duplicate_entities(
        ...
        try:
            ...
            duplicate_groups = self._cluster_similar_entities(
                embeddings, entities, threshold
            )
            ...
            logger.info(
                f"Found {len(duplicate_groups)} duplicate groups with {sum(len(g) for g in duplicate_groups)} total entities"
            )
    ...
    def _cluster_similar_entities(
        self, embeddings: np.ndarray, entities: list[dict[str, Any]], threshold: float
    ) -> list[list[dict[str, Any]]]:
    ...
    def cluster_similar_entities(
        ...
        try:
            ...
            logger.info(
                f"Clustered {len(entities)} entities into {len(cluster_results)} groups"
            )
    ...
    def semantic_search_in_graph(
        ...
        try:
            ...
            logger.info(
                f"Semantic graph search completed: {result.total_matches} matches"
            )
            return result
    ...
    def update_faiss_index(
        ...
        try:
            ...
            if hasattr(self.faiss_indexes["main"], "add_with_ids"):
                self.faiss_indexes["main"].add_with_ids(
                    new_embeddings, np.array(new_ids)
                )
            else:
            ...
            logger.info(
                f"Updated FAISS index with {len(new_embeddings)} new embeddings"
            )
    ...
    def reduce_dimensions_umap(
        ...
        try:
            ...
            logger.info(
                f"UMAP reduction completed: {embeddings.shape[1]} -> {n_components} dimensions"
            )
    ...
    def visualize_embeddings(
        ...
        try:
            ...
            if use_umap:
                reduced_embeddings = self.reduce_dimensions_umap(
            ...
            if labels:
                ...
                plt.scatter(
                    reduced_embeddings[:, 0], reduced_embeddings[:, 1], alpha=0.7
                )
    ...
    def evaluate_embedding_quality(
        self, embeddings: np.ndarray, texts: list[str], model_name: str
    ) -> QualityMetrics:
    ...
    def _calculate_coherence_score(self, embeddings: np.ndarray) -> float:
        ...
        try:
            ...
            return (
                float(np.mean(intra_cluster_similarities))
            ...
            for i in range(len(texts)):
                for j in range(i + 1, len(texts)):
                    ...
                    if len(words_i) > 0 and len(words_j) > 0:
                        text_similarity = len(words_i.intersection(words_j)) / len(
    ...
    def compare_models(
        ...
        try:
            ...
            for model_name in model_names:
                ...
                if not self._load_model(model_name):
                    logger.warning(
                        f"Skipping model {model_name} due to loading failure"
                    )
                    continue
                ...
                # Evaluate quality
                quality_metrics = self.evaluate_embedding_quality(
                    embeddings, texts, model_name
                )
            ...
            if results:
                ...
                for model_name, metrics in results.items():
                    logger.info(
                        f"  {model_name}: Coherence={metrics.coherence_score:.3f}, "
    ...
    def ensemble_similarity_search(
        self,
        query_text: str,
        candidate_texts: list[str],
        ...
        try:
                ...
                if model_name in all_embeddings:
                    ...
                    similarities = (
                        util.pytorch_cos_sim(
                            query_embedding, all_embeddings[model_name]
                        )[0]
                        .cpu()
                        .numpy()
                    )
            ...
            logger.info(
                f"Ensemble similarity search completed: {len(matches)} matches using {len(model_names)} models"
            )
            return result
