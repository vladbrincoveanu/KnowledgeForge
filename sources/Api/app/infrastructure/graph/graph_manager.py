...
from .models import Entity, Ontology, Relationship
...
class GraphManager:
    ...
    def store_ontology(self, ontology: Ontology, dataset_name: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully stored ontology with {len(ontology.entities)} entities and {len(ontology.relationships)} relationships"
            )
            return True
    ...
    def _store_relationship(
        self, session: Session, relationship: Relationship, dataset_name: str
    ):
        ...
        source_exists = session.run(
            entity_check_query, {"entity_id": relationship.source_entity_id}
        ).single()
        target_exists = session.run(
            entity_check_query, {"entity_id": relationship.target_entity_id}
        ).single()
    ...
    def _store_ontology_metadata(
        self, session: Session, ontology: Ontology, dataset_name: str
    ):
        """Store ontology metadata."""
        metadata_query = """
            ...
            with self.driver.session(database=self.database) as session:
                ...
                for record in result:
                    ...
                    if "attributes" in entity_data:
                        try:
                            entity_data["attributes"] = json.loads(
                ...
                for record in result:
                    if start_entity is None:
                        ...
                        if "attributes" in start_entity:
                            try:
                                start_entity["attributes"] = json.loads(
                    ...
                    if "attributes" in neighbor_data["entity"]:
                        try:
                            neighbor_data["entity"]["attributes"] = json.loads(
